...
import com.bigdata.market_analysis.beans.AdClickEvent;
import com.bigdata.market_analysis.beans.AdCountViewByProvince;
import com.bigdata.market_analysis.beans.BlackListUserWarning;
...
public class AdStatisticsByProvince {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 1.读取数据
        URL resource = AdStatisticsByProvince.class.getResource("/AdClickLog.txt");
    ...
    public static class FilterBlackListUser extends KeyedProcessFunction<Tuple, AdClickEvent, AdClickEvent> {
        ...
        public void processElement(AdClickEvent value, Context ctx, Collector<AdClickEvent> out) throws Exception {
            // 判断当前用户对同一广告的点击次数，如果不够上限，count+1正常输出。
            // 如果达到上限，直接过滤，并侧输出流输出黑名单报警
            Long curCount = countState.value();

            // 1.判断是否是第一个数，如果是，注册一个第二天0点的定时器
            if (curCount == 0) {
